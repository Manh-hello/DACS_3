...
import com.example.project.databinding.FragmentDangGiaoHangBinding
import com.example.projectmanage.Model.Entity.OrderDetail
import java.text.SimpleDateFormat
import java.util.Calendar
import java.util.Locale

class DangGiaoHangFragment : Fragment(), ClickBTNOrder {
    private lateinit var binding: FragmentDangGiaoHangBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentDangGiaoHangBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        myViewModel.listOrder.observe(viewLifecycleOwner) { orders ->
            ...
            myViewModel.loadOrderDetailOnDelivery(ids)
        }
        myViewModel.mapOrderDetails.observe(viewLifecycleOwner) { map ->
            val orders =
                myViewModel.listOrder.value.orEmpty().filter { map.containsKey(it.id) }
            binding.progress.visibility = View.GONE
            adapter = OrderAdapter(orders.toMutableList(), map, this@DangGiaoHangFragment)
            if (orders.isEmpty()) {
    ...
    override fun clickSuccess(id: String) {
        ...
        bellViewModel.save(cusId, "Nhận Hàng", "Nhận hàng thành công", "Đơn hàng của bạn đã được nhận, bạn có thể xem lại tại đây", formartDate)
        adapter.notifyDataSetChanged()
    }
